...
### Versioning
The current CI setup will skip chart releases releases for PRs that don't make any changes to the generated chart.
...
The `appVersion` field in the starter's `Chart.yaml` is the source of truth for the version of AWX Operator that will be pulled into the chart templates.

> Before version 3.0.0 of this Chart, the `version` field was tied to the AWX Operator version. Now, these are separate fields which can be incremented to reflect changes to the chart as well as the underlying app it installs.
